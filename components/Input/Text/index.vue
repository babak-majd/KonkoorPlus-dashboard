<template>
  <div class="relative flex items-center justify-center h-full w-full">
    <input :id="inputId" :title="title" :autocomplete="autocomplete" :value="value" :type="isVisible ? 'text' : type"
      ...
      class="resize-none bg-white w-full border border-b-[2px] border-base-400 focus:border-b-primary rounded-[3px] text-sm placeholder:text-sm p-2 h-10 text-base-content placeholder:text-base-400" />
    <span v-if="type === 'password' && value !== ''" class="cursor-pointer lg:hidden absolute right-2" @click="isVisible = !isVisible">
