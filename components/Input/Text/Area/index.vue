...
<template>
  <div class="flex flex-col items-start">
    <textarea :value="modelValue" @input="$emit('update:modelValue', $event.target.value)" :placeholder="placeholder"
      ...
      :class="used === null || used === 0 ? 'focus:border-b-main' : 'focus:border-b-base-400'"></textarea>
    <span v-if="used !== null" class="block h-[2px] transition-all ease-in-out duration-300 z-10 w-full -mt-[2px]"
      :class="[use_color]" :style="`width: ${used <= 100 ? used : 100}%;`">
    </span>
  </div>
</template>
